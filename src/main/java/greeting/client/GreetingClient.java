...
public class GreetingClient {
    ...
    private static void doGreet(ManagedChannel channel) {
        System.out.println("doGreet function ->");
        GreetingServiceGrpc.GreetingServiceBlockingStub stub = GreetingServiceGrpc.newBlockingStub(channel);
        GreetingResponse response = stub.greet(GreetingRequest.newBuilder().setFirstName("Rômulo").build());

        System.out.println("Response: " + response.getResult());
    }

    private static void doGreetManyTimes(ManagedChannel channel) {
        System.out.println("doGreetManyTimes function ->");
        GreetingServiceGrpc.GreetingServiceBlockingStub stub = GreetingServiceGrpc.newBlockingStub(channel);

        stub.greetManyTimes(GreetingRequest.newBuilder().setFirstName("Rômulo").build()).forEachRemaining(
            greetingResponse -> {
                System.out.println("Response-mt: " + greetingResponse.getResult());
            }
        );
    ...
    private static void doLongGreet(ManagedChannel channel) throws InterruptedException {
        System.out.println("doLongGreet function ->");
        GreetingServiceGrpc.GreetingServiceStub stub = GreetingServiceGrpc.newStub(channel);

        List<String> names = new ArrayList<>();
        CountDownLatch latch = new CountDownLatch(1);

        Collections.addAll(names, "Huguinho", "Zezinho", "Luizinho", "João", "José");
    ...
    private static void doGreetEveyrone(ManagedChannel channel) throws InterruptedException {
        System.out.println("doGreetEveyrone function ->");
        GreetingServiceGrpc.GreetingServiceStub stub = GreetingServiceGrpc.newStub(channel);
        CountDownLatch latch = new CountDownLatch(1);
        ...
        Arrays.asList("Huguinho", "Zezinho", "Luizinho", "João", "José").forEach(name ->
            stream.onNext(GreetingRequest.newBuilder().setFirstName(name).build())
        );
    ...
    private static void doGreetWithDeadline(ManagedChannel channel) {
        System.out.println("doGreetWithDeadline ->");
        GreetingServiceGrpc.GreetingServiceBlockingStub stub = GreetingServiceGrpc.newBlockingStub(channel);
        GreetingRequest request = GreetingRequest.newBuilder().setFirstName("Romilo").build();
