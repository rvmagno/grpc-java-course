...
public final class BlogServiceImpl extends BlogServiceGrpc.BlogServiceImplBase {
    ...
    private StatusRuntimeException error(Status status, String message) {
        return status.withDescription(message).asRuntimeException();
    }

    @SuppressWarnings("SameParameterValue")
    private StatusRuntimeException error(Status status, String message, String augmentMessage) {
...
//        Document result = mongoCollection.find(eq("_id", new ObjectId(id))).first();
        Optional<Document> result = Optional.ofNullable(mongoCollection.find(eq("_id", new ObjectId(id))).first());

        if (result.isEmpty()) {
            System.out.println("Blog not found");
            responseObserver.onError(
                error(Status.NOT_FOUND,BLOG_WAS_NOT_FOUND,"BlogId: " + id)
            );
            return;
        }

        System.out.println("Blog found, sending response");
        responseObserver.onNext(documentToBlog(result.get()));
        responseObserver.onCompleted();
    }
    ...
    public void updateBlog(Blog request, StreamObserver<Empty> responseObserver) {
        ...
        Optional<Document> optDoc = Optional.ofNullable(result);
